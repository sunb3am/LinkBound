...
Status: revised after scope and access review, 2026-09-23. Releases 0 through C and the bounded Phase D collector are deployed for Shubham. See the [current Phase D record](linkbound-phase-d-progress.md) for verified coverage and remaining work. This is a focused internal operations tool, not a general CRM platform.
...
- The [LinkedIn access risk review](linkbound-linkedin-access-risk-review.md) records LinkedIn's published device, network, cookie, challenge, and activity signals against the local and hosted browser setup. The operator authorized internal hosted use of Shubham's account. A bounded hosted send regression remains before live sends are enabled; there is no documented safe fingerprint or detection threshold to implement.
...
Run the app and browser viewer through private Tailscale Serve HTTPS endpoints. The [access options review](linkbound-access-options.md) records this choice. Enroll the Linode and internal devices, grant direct tailnet members and tagged devices access to ports 443 and 8443, and run Serve in persistent background mode. Bind FastAPI, websockify, and raw VNC to loopback. Do not use Funnel or expose Chrome DevTools. Tailnet membership authorizes the UI, WebSocket, export, download, and run controls through the local Serve proxy; there is no separate per-login app allowlist. Keep SSH grants narrower. A separate scoped service credential would handle a later Cruitical integration. Chrome profile directories are credentials; restrict permissions and keep them outside the repository. Back up the database and attachments to an encrypted offsite location, and handle profile backups as credential material. Verify restore, not just backup creation.
...
| B. Private Linode app | Tailscale Serve for app and viewer, tailnet-authorized UI/API, systemd deployment, persistent SQLite and files, backups and restore | Non-tailnet UI/API/file/WebSocket denied; failed release can roll back before work resumes; restart preserves CRM and LinkedIn login; restore succeeds |
...
| Operator authorization for internal use of Shubham's account | Given on 2026-09-23; no separate approval workflow. A controlled hosted send regression remains before enabling live sends. |
